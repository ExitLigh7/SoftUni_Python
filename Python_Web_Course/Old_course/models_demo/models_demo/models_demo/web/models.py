...
class AuditInfoMixin(models.Model):
    class Meta:
        # 1. when it's abstract no table will be created in the DB
        # 2. Can be inherited by other models
        abstract = True
...
class Department(AuditInfoMixin, models.Model):
    ...
    def get_absolute_url(self):
        reverse('department_detail', kwargs={
            'pk': self.pk,
            'slug': self.slug,
        })
...
class Employee(AuditInfoMixin, models.Model):
    ...
    # class Meta can also be is used for default values
    class Meta:
